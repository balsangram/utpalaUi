...
function Add_Therapists() {
  const navigate = useNavigate();
  const [isSaving, setIsSaving] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [activeSection, setActiveSection] = useState("personal");

  // Initial empty therapist data for creation
  ...
  const handleSave = async () => {
    ...
    console.log("Created Therapist Data:", therapist);
    setIsSaving(false);
    setShowSuccess(true);

    setTimeout(() => {
      setShowSuccess(false);
      navigate("/admin/therapists"); // Redirect to therapists list
    }, 2000);
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-a)" }}>
      ...
      <HeadingCard
        ...
        subtitle="Create a new therapist profile with complete information"
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-3">
              ...
              {/* Quick Preview Placeholder (for new therapist) */}
              <div
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <FormInput
                        label="Department"
                        icon={Building}
                        value={therapist.department}
                        onChange={(e) => updateField("department", e.target.value)}
                        placeholder="e.g., Rehabilitation Services, Pediatric Therapy"
                      />
                      <FormInput
                    ...
                    <div>
                      ...
                      <FormTextArea
                        label="Therapy Degrees & Training"
                        icon={GraduationCap}
                        value={therapist.qualifications}
                        onChange={(e) => updateField("qualifications", e.target.value)}
                        rows={3}
                        placeholder="Enter degrees and training separated by commas, e.g., DPT, OTR/L, SLP"
                      />
                    </div>
                  </div>
                )}
